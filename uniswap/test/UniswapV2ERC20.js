...
const expandTo18Decimals = (n) => env.ethers.getBigInt(n) * env.ethers.getBigInt('1000000000000000000');
...
describe('UniswapV2ERC20 Happy Path', () => {
    ...
    it("name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH", async () => {
        ...
        expect(await token.DOMAIN_SEPARATOR())
            .to.eq(ethers.keccak256(abiCoder.encode(["bytes32", "bytes32", "bytes32", "uint256", "address"], [
        ...
        expect(await token.PERMIT_TYPEHASH())
            .to.eq(ethers.keccak256(ethers.toUtf8Bytes("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)")));
    });
    it("approve", async () => {
    ...
    it("transferFrom", async () => {
        ...
        expect(await token.allowance(await wallet.getAddress(), await other.getAddress()))
            .to.eq(TEST_AMOUNT);
